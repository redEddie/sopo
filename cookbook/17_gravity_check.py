...
토크 OFF → 자세별 측정/예측 비교표. 반복 후 q로 종료하면 외력 추정 모드(선택) 진입.
...
def load_arm(path: str) -> dict:
    """arm.yaml에서 관절명 -> 모터 정보. ref는 관절 각도 기준 모터."""
    data = yaml.safe_load(Path(path).read_text())
    joints = {}
    for j in data["joints"]:
        if j["type"] == "dual":
            joints[j["name"]] = {"ids": j["ids"], "ref": j["reference_id"]}
        else:
            joints[j["name"]] = {"ids": [j["motor_id"]], "ref": j["motor_id"]}
    return joints


def load_gravity_config(path: Path) -> dict:
    data = yaml.safe_load(path.read_text()) if path.exists() else {}
    return (data or {}).get("gravity", {})
...
def run_scale_calibration(bus, gm, joints, names, all_ids, read_q, args, calib_path, grav):
    ...
    """
    from sopo.dynamics import JOINT_MAP
    ...
    scale = {}
    for n in names:
        loads = [load1[i] - load0[i] for i in joints[n]["ids"]]
        # 미러 부호 반전을 흡수: 관절 측정 Δ는 모터별 |Δ|의 평균
        measured = sum(abs(d) for d in loads) / len(loads)
        pred_pm = delta_nm[n] / JOINT_MAP[n]["stall"] * 1000
...
def main() -> None:
    ...
    joints = load_arm(args.config)
    names = list(joints)                      # J1..J6 (arm.yaml 순서)
    ref_ids = [joints[n]["ref"] for n in names]
    ...
    if args.calibrate_vertical:
        ...
        zero = {n: pos[joints[n]["ref"]] for n in names}
        dirs = load_gravity_config(calib_path).get("dir") or {n: 1 for n in names}
        save_zero_ticks(calib_path, zero, dirs)
        print("URDF zero 기준 저장 완료. 이후 자세 비교는 인자 없이 실행하세요.")
        bus.disconnect()
        return

    grav = load_gravity_config(calib_path)
    if "zero_ticks" not in grav:
        print("zero 기준이 없습니다. 먼저 --calibrate-vertical 실행 (팔을 직립 자세로).")
        bus.disconnect()
        return
    zero_ticks = grav["zero_ticks"]
    dirs = grav.get("dir") or {n: 1 for n in names}
    scale = grav.get("scale") or {}

    gm = GravityModel()

    def read_q(pos):
        return {n: ticks_to_rad(pos[joints[n]["ref"]], zero_ticks[n], dirs.get(n, 1))
                for n in names}

    apply_safety(bus, all_ids, SafetyLimits(torque_limit=args.hold_torque))

    if args.scale:
        run_scale_calibration(bus, gm, joints, names, all_ids, read_q,
                              args, calib_path, grav)
        bus.disconnect()
        return

    pose = 0
    history = []  # (측정‰ 관절별, 예측‰ 관절별)
    try:
        while True:
            ...
            pred_pm = gm.load_permille(q, scale)

            def joint_ext(n):
                """외력 [N·m]: 측정 합성 토크 − G. 듀얼은 미러 부호를 자동 판별."""
                ids = joints[n]["ids"]
                stall = JOINT_MAP[n]["stall"]
                s = scale.get(n, 1.0)
                if len(ids) == 1:
                    est = load[ids[0]] * stall / (1000 * s)
                    return est - pred_nm[n], est
                best = None
                for signs in ((1, -1), (-1, 1)):
                    # 모터별 기여 합산 (stall은 관절 합산이므로 per-motor는 절반)
                    est = sum(sg * load[i] for sg, i in zip(signs, ids)) * stall / (2000 * s)
                    err = abs(est - pred_nm[n])
                    if best is None or err < best[0]:
                        best = (err, est)
                return best[1] - pred_nm[n], best[1]

            print(f"\n{'관절':>4} | {'측정‰':>14} | {'예측‰':>8} | {'예측N·m':>8} | {'외력N·m':>8}")
            for n in names:
                loads = "/".join(f"{load[i]:+.0f}" for i in joints[n]["ids"])
                ext, _ = joint_ext(n)
                print(f"{n:>4} | {loads:>14} | {pred_pm[n]:+8.0f} | {pred_nm[n]:+8.3f} | {ext:+8.3f}")
    ...
    if len(history) >= 2:
        ...
        for n in names:
            ...
            if len(ids) == 1:
                ...
                # 듀얼: 미러 장착 부호 두 가지를 모두 평가해 예측과 잘 맞는 쪽을 선택
                best = None
                for signs in ((1, -1), (-1, 1)):
                    err = 0.0
                    for m, p in history:
                        err += sum(abs(s * m[i] - p[n]) for s, i in zip(signs, ids))
                    if best is None or err < best[0]:
                        best = (err, signs)
                err, signs = best
