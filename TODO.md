...
- **EPROM Max_Torque_Limit = 상한 600 (#10)** — 10_persist_caps가 마지막으로 기록한 값은 운용 캡(200~300)이므로 **다시 실행 필요** (안 하면 홀드 캡 600이 EPROM 상한에 막힐 수 있고 데몬이 시작 시 드리프트 경고)
- J2 캡: calibration은 300 유지 중. 최저 자세에서 자중을 못 들어 init/goto가 COLLISION으로 멈춤 → 400 권장(사용자 결정 대기)
